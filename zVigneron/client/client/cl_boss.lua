	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(mainMenu,function() 
			...
			if societyvigne~= nil then
                RageUI.Button('Argent de la société :', " ", {RightLabel = "~g~"..societyvigne.."$"}, true, {onSelected = function()end});   
            end
            ...
            RageUI.Button('Retirer de l\'argent', " ", {RightLabel = ">"}, true, {onSelected = function()
                ...
                TriggerServerEvent("vigne:withdrawMoney","society_"..ESX.PlayerData.job.name ,money)
                RefreshMoney()
            end});   

            RageUI.Button('Déposer de l\'argent', " ", {RightLabel = ">"}, true, {onSelected = function()
                local money = KeyboardInput('Combien voulez vous retirer :', '', 10)
                TriggerServerEvent("vigne:depositMoney","society_"..ESX.PlayerData.job.name ,money)
                RefreshMoney()
            end});  
...
function RefreshMoney()
    if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then
        ESX.TriggerServerCallback('vigne:getSocietyMoney', function(money)
            societyvigne = money
        end, "society_"..ESX.PlayerData.job.name)
    end
end

function Updatessocietyvignemoney(money)
    societyvigne = ESX.Math.GroupDigits(money)
